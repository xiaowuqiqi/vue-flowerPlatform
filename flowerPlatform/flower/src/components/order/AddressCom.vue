<template>
  <div class="addr-list-wrap">
    ...
    <div class="addr-list">
      <ul>
        <li class="transitionf10" v-for="(item,index) in addressListFilter" :class="{check:selAddressId==item._id}"
          ...
          <div class="addr-opration addr-del">
            <a href="javascript:;" class="addr-del-btn" @click="delAddressId=item._id;mdShow2=true">
        ...
        <li class="addr-new transitionf10" @click="mdShow =true">
    ...
    <div class="shipping-addr-more ">
      <a class="addr-more-btn up-down-btn f10red" href="javascript:;" @click="expand" :class="{open:!(limit==3)}">
        展开
        <i class="i-up-down">
          <i class="fa fa-sort-desc"></i>
          <i class="fa fa-sort-up" v-show="false"></i>
        </i>
      </a>
    </div>
    <!--模态框-->
    <modal :mdShow="mdShow" @closemodal="mdShow=false">
      <h2 slot="h2">添加收花人信息</h2>
      <label slot="input">电话：
        <input id="addressAddTel" type="tel" placeholder="" @blur="verifica('tel',$event)"><i v-show="verificaDate.checked==0">{{verificaDate.str}}</i>
      </label>
      <label slot="input">姓名：
        <input id="addressAddName" type="text" placeholder="" @blur="verifica('name',$event)"><i v-show="verificaDate.checked==1">{{verificaDate.str}}</i>
      </label>
      <label slot="input">地址：
        <input id="addressAddAdd" type="text" placeholder="" @blur="verifica('add',$event)"><i v-show="verificaDate.checked==2">{{verificaDate.str}}</i>
      </label>
      <span slot="span" class="f10red">[小标题很小]</span>
      <div slot="btn">
        <a class="wzBtnRed2" @click="toPushAddress">确认</a>
        <a class="wzBtn2" @click="mdShow=false">关闭</a>
      </div>
    </modal>
    <modal :mdShow="mdShow2" @closemodal="mdShow2=false">
      <h2 slot="h2">确定删除此地址么？</h2>
      <span slot="span" class="f10red">[点击确定删除地址信息]</span>
      <div slot="btn">
        <a class="wzBtnRed2" @click="toDelAddress">确认</a>
        <a class="wzBtn2" @click="mdShow2=false">关闭</a>
      </div>
    </modal>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      init() {
        this.$http.get('/serverUsers/address').then((response) => {
          let res = response.data
          if (res.status === '0')
            this.addressList = res.result.recAddress
        })
      },
      //好用的展开功能
      expand() {
      ...
      verifica(name,e){
        ...
        this.verificaDate = verobj
      },
      //添加地址
      toPushAddress(){
        if(this.verificaDate.checked===200){
          //输入成功
          let addName = $('#addressAddName').val()
          let addTel = $('#addressAddTel').val()
          let address = $('#addressAddAdd').val()

          this.$http.post('/serverUsers/pushAddress',{
            addName:addName,
            addTel:addTel,
            address:address
          }).then((response) => {
      ...
      toDelAddress(){
        ...
        this.$http.post('/serverUsers/delAddress',{
          addressId:addressId
        }).then((response) => {
          let res = response.data
          if (res.status === '0'){
      ...
      //点击选择
      toSelect(item,index){
        this.selAddressId=item._id
        this.$emit("getaddressid",item._id)//此处closemodal不能大写
      }
    }
  }
</script>
