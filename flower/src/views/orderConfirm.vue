      ...
      <div class="checkout-order">
        <wow-plus :delay="0.2">
          ...
          <div class="page-title-normal checkout-title">
            <h2><span>查看订单无误后，请点击支付</span></h2>
          </div>
        </wow-plus>
        <order-goods-item :orderData="conList">
          <thead slot="thead">
          <th>##</th>
          <th>单价</th>
          <th>数量</th>
        ...
        <div class="order-foot-wrap">
          <div class="prev-btn-wrap">
            <button class="wzBtn">上一步</button>
          </div>
          <div class="next-btn-wrap">
            <button class="wzBtnRed" @click="payMent">支付</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getOrderTotal() {
        let pr = 0
        this.conList.orderGoods.forEach(function (item) {
          pr += item.goodsNum * item.goods.base.price
        })
        this.orderTotal = pr
        return pr
      },
    ...
    methods: {
      init() {
        this.$http.get('/serverUsers/temporary').then((response) => {
      ...
      payMent() {
        ...
        //存入后端生成订单
        this.$http.post('/serverUsers/pushOrder').then((response) => {
          let res = response.data
          if (res.status === "0") {
            this.$router.push({
              path: '/orderSuccess?orderId=xxx'
            })
          }
        })
      }
