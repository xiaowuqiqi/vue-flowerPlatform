<template>
  <div>
              ...
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'check':item.goodsCheck}"
                  ...
                  <div class="cart-item-pic">
                    <img :src="item.goods.base.CoverImgUrl">
                  </div>
                  <div class="cart-item-title">
                    <div class="item-name"><a :href="'/#/detail?goodsId='+item.goodsId" class="f333">{{item.goods.base.goodsName}}</a></div>
                  </div>
                </div>
                <div class="cart-tab-2">
                  <div class="item-price">{{item.goods.base.price}}</div>
                </div>
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        ...
                        <span class="select-ipt">{{item.goodsNum}}</span>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.goodsNum*item.goods.base.price|currency('￥')}}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delCartConfirm(item.goodsId)">
                      <!--删除购物车-->
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                  <a class="wzBtn" @click="toggleCheckAll()">全选 All</a>
              </div>
            </div>
            <div class="cart-foot-r">
              <div class="item-total">
                总 计: <span class="total-price">{{totalPrice|currency('￥',2)}}</span>
                <!--|后面就是过滤器，可以用来格式化数据。
                当|前面运算完后的值，会放入|后面的方法的第一个参数中-->
                <!--currency参数（格式化的值，金额前面的标识符，小数点后几位）-->
              </div>
              <div class="btn-wrap submit">
                <a class="wzBtnRed2" @click="toAddress">生成订单</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <modal :mdShow="mdShow" @closemodal="mdShow=false">
      <h2 slot="h2">确认删除么？</h2>
      <span slot="span" class="f10red">[点击确认，删除鲜花]</span>
      <div slot="btn">
        <a class="wzBtnRed2" @click="delCart">确认</a>
        <a class="wzBtn2" @click="mdShow=false">关闭</a>
      </div>
    </modal>
    <modal :mdShow="mdShow2" @closemodal="mdShow2=false">
      <h2 slot="h2">您还没有选择商品</h2>
      <span slot="span" class="f10red">[点击商品前面的复选框，选择商品]</span>
      <div slot="btn">
...
<script>
  import '../assets/css/order/checkout.css'
  ...
  export default {
    ...
    computed:{
      ...
      totalPrice(){
        ...
        this.cartList.forEach((item)=>{
          if(item.goodsCheck == '1'){
            money += parseFloat(item.goods.base.price)*parseFloat(item.goodsNum)
          }
        })
        return money
    ...
    methods:{
      ...
      toggleCheckAll(){
        ...
        this.cartList.forEach((item)=>{
          item.goodsCheck = this.checkAllFlag
        })
        this.$http.post('/serverUsers/checkAllCart',{
      ...
      editCart(flag,item){//加减数量,是否选中
        ...
        //发送请求
        this.$http.post("/serverUsers/editCart",{
          goodsId:item.goodsId,
          goodsNum:item.goodsNum,
          goodsCheck:item.goodsCheck
        }).then((response)=>{
          let res = response.data
          console.log(res)
        })
      ...
      init(){
        this.$http.get("/serverUsers/cart").then((response)=>{
          let res = response.data
          if(res.status == '0')
          this.cartList = res.result.cart
          else if(res.status == '-1'){
            this.cartList = null
          }
        })
      },
      delCartConfirm(delGoodsId){
        this.mdShow=true
        this.delGoodsId = delGoodsId
      },
      delCart(){
        this.$http.post("/serverUsers/delcart",{
